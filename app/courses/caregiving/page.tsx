...
const course = courses.find((c) => c.slug === 'caregiving')!;
      ...
      <section className="py-20">
        <Container className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {course.careerPaths && (
              <div className="rounded-3xl border border-slate-100 bg-white p-8 shadow-soft">
                <h2 className="text-2xl font-bold text-brand-dark">Career Paths</h2>
                <div className="mt-4 h-0.5 w-10 rounded-full bg-brand-gold" />
                <div className="mt-5 flex flex-wrap gap-3">
                  {course.careerPaths.map((c) => (
                    <span key={c} className="rounded-full bg-brand-green/10 px-4 py-2 text-sm font-medium text-brand-green">{c}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="space-y-6">
            ...
            {course.accreditation && (
              <div className="rounded-3xl border border-brand-gold/20 bg-brand-gold/5 p-6">
                <h3 className="text-lg font-bold text-brand-dark">Accreditation</h3>
                <div className="mt-3 h-0.5 w-8 rounded-full bg-brand-gold" />
                <ul className="mt-4 space-y-2">
