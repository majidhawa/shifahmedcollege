      ...
      <div className="bg-brand-dark">
        <div className="container-shell flex items-center justify-between py-2">
          <p className="text-[11px] tracking-widest text-white/50 uppercase">
            Train with purpose. Lead with care. Join us
          </p>
        ...
        <div className="container-shell flex items-center justify-between border-b border-slate-100 py-3">
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a href="tel:+254700000000"
              className="flex items-center gap-2 rounded-full border border-slate-200 px-4 py-1.5 text-xs text-slate-600 transition-all duration-200 hover:border-brand-green/40 hover:bg-brand-green/5 hover:text-brand-green">
            ...
            <a href="mailto:admissions@yourdomain.co.ke"
              className="flex items-center gap-2 rounded-full border border-slate-200 px-4 py-1.5 text-xs text-slate-600 transition-all duration-200 hover:border-brand-green/40 hover:bg-brand-green/5 hover:text-brand-green">
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`group relative px-4 py-4 text-sm font-medium transition-colors duration-200 ${
                    active ? 'text-brand-green' : 'text-slate-600 hover:text-brand-green'
                  }`}
                >
                  {link.label}
                  {/* animated underline */}
                  <span className={`absolute bottom-0 left-3 right-3 h-[2px] rounded-full bg-brand-gold transition-all duration-300 ${
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-400 ease-in-out ${menuOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
        <div className="border-t border-slate-100 bg-white shadow-xl">
          <nav className="container-shell flex flex-col py-4 gap-1">
            ...
            <Link
              href="/apply"
              className="mt-3 flex items-center justify-center gap-2 rounded-full bg-brand-green py-3 text-sm font-semibold text-white shadow-md shadow-brand-green/20 transition hover:bg-brand-dark"
            >
              Apply Now
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </Link>
