      ...
      <div className="container-shell relative z-10 flex flex-1 items-center py-24">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-[0.22em] text-brand-gold">{site.intake}</span>
          </div>

          <h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight md:text-6xl">{slide.title}</h1>
          <div className="mt-4 h-1 w-16 rounded-full bg-brand-gold" />
          <p className="mt-5 max-w-xl text-base leading-7 text-white/80 md:text-lg">{slide.text}</p>

          <div className="mt-8 flex flex-wrap gap-4">
            <Link href="/apply" className="rounded-full bg-brand-green px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-brand-green/30 transition hover:bg-brand-dark hover:shadow-brand-green/10">
              Apply Now →
            </Link>
            <Link href="/courses" className="rounded-full border border-white/40 bg-white/10 px-7 py-3.5 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20">
              Explore Courses
            </Link>
          </div>
